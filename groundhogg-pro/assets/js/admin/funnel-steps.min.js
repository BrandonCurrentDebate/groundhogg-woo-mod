($=>{const{select,input,inputRepeater,loadingModal,toggle,dialog,inputWithReplacements,bold}=Groundhogg.element;const{metaPicker}=Groundhogg.pickers;const{post,get}=Groundhogg.api;const{sprintf,__,_x,_n}=wp.i18n;const EditMeta={edit:({meta,data})=>{const{changes:changes=[]}=meta;return[`<p><b>${__("Meta Changes")}</b></p>`,`<div class="changes"></div>`,"<p></p>"].join("")},onMount:({ID,meta:meta={}},updateStepMeta)=>{let id=`step_${ID}_edit_meta_settings`;let{changes:changes=[]}=meta;if(!Array.isArray(changes)){changes=[]}inputRepeater(`#${id} .changes`,{rows:changes,sortable:true,cells:[props=>input({placeholder:__("Meta"),className:"input meta-picker",...props}),({value,...props})=>select({...props,selected:value,style:{width:"auto"},options:{set:"Set",add:"Add",subtract:"Subtract",multiply:"Multiply",divide:"Divide",delete:"Delete"}}),(props,[key,opp,val])=>input({placeholder:__("Value"),readonly:opp==="delete",...props})],onMount:()=>{metaPicker(`#${id} .meta-picker`);$(`#${id} select`).on("change",e=>{if(e.target.value==="delete"){e.target.nextElementSibling.setAttribute("readonly",true)}else{e.target.nextElementSibling.removeAttribute("readonly")}})},onChange:rows=>{changes=rows;updateStepMeta({changes:changes})}}).mount()}};const WebhookListener={edit:({meta,data})=>{const{body:body=[],response_type:response_type="none"}=meta;return[select({name:"response_type",selected:response_type,options:{none:"No Response",contact:"The Contact Record",json:"Custom JSON"}}),response_type==="json"?`<p><b>${__("Response Body")}</b></p>`:"",response_type==="json"?`<div class="body"></div>`:"","<p></p>"].join("")},onMount:({ID,meta},updateStepMeta)=>{let id=`step_${ID}_webhook_listener_settings`;let{body:body=[],response_type:response_type="none"}=meta;$(`#${id} [name=response_type]`).on("change",e=>{updateStepMeta({response_type:e.target.value},true)});if(response_type==="json"){inputRepeater(`#${id} .body`,{rows:body??[],sortable:true,cells:[input,input],onChange:rows=>{body=rows;updateStepMeta({body:body})}}).mount()}}};const Webhook={edit:({meta,data})=>{const{headers:headers=[],body:body=[],method:method="post",post_url:post_url="",content_type:content_type=""}=meta;return[`<p><b>${__("Target URL")}</b></p>`,`
