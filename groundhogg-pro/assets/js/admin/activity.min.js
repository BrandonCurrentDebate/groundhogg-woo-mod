(()=>{const{bold}=Groundhogg.element;const{superlinks:Superlinks}=Groundhogg.stores;const{sprintf,__,_x,_n}=wp.i18n;Groundhogg.ActivityTimeline.addType("clicked_superlink",{iconFramed:true,icon:`
...
</svg>`,render:({meta,data})=>{let{superlink:superlink=0}=meta;return`${sprintf(__("Clicked superlink %s","groundhogg-pro"),bold(Superlinks.get(superlink)?.data.name))}</a>`},preload:({meta,data})=>{let{superlink:superlink=0}=meta;if(superlink){return Superlinks.maybeFetchItem(superlink)}}})})();